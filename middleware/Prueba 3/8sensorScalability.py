...
def store_data(block_id, sensor_data, ttn_time=None, confirmation_time=None):
    """Store data in CSV file"""
    try:
        file_exists = os.path.isfile('iota_data_8sensors.csv')  
        
        if not confirmation_time:
            with open('iota_data_8sensors.csv', mode='a', newline='') as file: 
                writer = csv.writer(file)
                if not file_exists:
                    writer.writerow([
                        'Block ID', 'Device ID', 'Timestamp', 
                        'AHT10 Temperature', 'AHT10 Humidity', 
                        'DS18B20_1 Temperature', 'Light level 1',
                        'Soil moisture 1', 'DS18B20_2 Temperature',
                        'Light level 2', 'Soil moisture 2',
            ...
            df = pd.read_csv('iota_data_8sensors.csv', dtype={
                'Block ID': str,
                'Device ID': str,
                'Timestamp': str,
            ...
            df.to_csv('iota_data_8sensors.csv', index=False)
...
def plot_response_times():
   ...
   try:
       if not os.path.exists('iota_data_8sensors.csv'):  
           print("No data file found.")
           return
           
       # Read data from CSV
       df = pd.read_csv('iota_data_8sensors.csv')  
       ...
       plt.title('Response time: TTN to Tangle (8 Sensors)')
       plt.xlabel('Time')
       ...
       # Save plot
       plt.savefig('response_times_8sensors.png', 
                  dpi=300, 
                  bbox_inches='tight',
                  facecolor='white',
                  edgecolor='none')
       ...
       print(f"Graph saved as 'response_times_8sensors.png'")
    ...
    try:
        return {
            ...
            "measurements": {
                ...
                "ds18b20_temp_1": payload["uplink_message"]["decoded_payload"]["v2"],
                "light_level_1": payload["uplink_message"]["decoded_payload"]["v3"],
                "soil_moisture_1": payload["uplink_message"]["decoded_payload"]["v4"],
                "ds18b20_temp_2": payload["uplink_message"]["decoded_payload"]["v5"],
                "light_level_2": payload["uplink_message"]["decoded_payload"]["v6"],
                "soil_moisture_2": payload["uplink_message"]["decoded_payload"]["v7"]
            },
            "metadata": {
...
def send_to_iota(sensor_data, ttn_time, device_id):
    ...
    try:
        ...
        print("\nSensor readings:")
        print(f"AHT10 Temperature: {sensor_data['measurements']['aht10_temperature']}°C")
        print(f"AHT10 Humidity: {sensor_data['measurements']['aht10_humidity']}%")
        print(f"DS18B20 Temperature 1: {sensor_data['measurements']['ds18b20_temp_1']}°C")
        ...
        print(f"DS18B20 Temperature 2: {sensor_data['measurements']['ds18b20_temp_2']}°C")
